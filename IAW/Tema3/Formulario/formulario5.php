...
<?php
   ...
   $password = recoge("usuario_pass");
   $idioma = recoge("usuario_idioma");
   $pais = recoge("usuario_pais");
   $telefono = recoge("usuario_telefono");
   $edad = recoge("usuario_edad");
   $intereses = recoge("usuario_intereses",[]);
   
   //regla
   $reglaNombre = "/^[a-zA-Z0-9.-_]{2,}$/";
   $reglaEmail = "/^[a-zA-Z0-9]+@[a-zA-Z0-9]+\.[a-zA-Z]{2,}$/";
   $reglaPassword = "/^[a-zA-Z0-9.-_]{8,}$/";

   $reglaTelefono = "/^[0-9]{9}$/";
   $reglaFechaNace = "/^[1-3]?[0-9]-()$/";
   $reglaIdioma = "/es|en/";
   $reglaPais = "/es|mx|ar|co/";

   //check error
   $errores="";
   if(!preg_match($reglaNombre,$nombre))
      {$errores.="<li>Debes introducir un nombre valido</li>";}
   if(!preg_match($reglaEmail,$email))
      {$errores.="<li>Debes introducir tu email en forma correcta</li>";}
   if(!preg_match($reglaPassword,$password))
      {$errores.="<li>Debes escribir un password que tiene longitud más que 8</li>";}

   if(!preg_match($reglaTelefono,$telefono))
      {$errores.="<li>Debes escribir un telefono que tiene longitud de 9</li>";}
   if(!preg_match($reglaIdioma,$idioma))
      {$errores.="<li>Debes marcar idioma</li>";}
   if(!preg_match($reglaPais,$pais))
      {$errores.="<li>Debes marcar pais</li>";}
   if(empty($intereses))
      {$errores.="<li>Debes marcar un interés</li>";}
   if($edad<18||$edad>100)
      {$errores.="<li>Debes escribir el edad entre 18 y 100</li>";}
